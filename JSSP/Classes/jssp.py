...
class JSSP():
    ...
    def format_data(self, times_mat, machines_mat):
        ...
        #self.solve(self.operations, self.machines, self.jobs, no_machines, no_jobs)

    def random_solution(self):
        # Se calcula un makespan inicial solamente con el recorrido hacia adelante
        self.g, self.ms, self.jg, self.mg, self.operations = self.dg.find_makespan(self.jobs, self.operations, self.machines, self.no_machines)

    def create_neighbor(self):
        self.g, self.ms, self.jg, self.mg, self.operations = self.dg.perturbate_solution(self.jg, self.mg, self.operations)

    def plot(self):
        self.solution.plot_solution(g, no_machines, machines, operations, ms, no_jobs)
        #self.solution.print_solution(g)
