...
class Plotter:
    ...
    def plot_solution(self, graph, no_machines, machines, operations, ms, no_jobs):
        ...
        for k, v in graph.items():
            ...
            patch_list.append(patches.Rectangle(coordinates, v[0].get_duration()/shortener, 1, facecolor='#'+str(color)))
        ...
        for i in range(no_machines*2):
            if i%2 != 0:
             ax.annotate('M ' + str(m_indexes.pop()), xy=(0, i/2), xytext=(-1*int(float(ms))/(10*shortener), i/2 + 0.9), size = 15)
