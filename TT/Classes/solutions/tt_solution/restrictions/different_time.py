...
class DifferentTime(Restriction):
    def is_violated(self, solution, e) -> bool:
        uc = solution.state["classes"][e.uid]
        events = solution.solution_space
        
        if "DIFF_TIME" in uc.properties['constraints']:
            related_classes_ids = uc.properties['constraints']["DIFF_TIME"]
                    ...
                    for event in event_list:
                        ...
                        if event.day == e.day:
                            if (e.start_time < event.start_time + event.duration and
                                ...
                                print("diff time broken by {0} and {1}".format(e.uid, event.uid))
                                return True
